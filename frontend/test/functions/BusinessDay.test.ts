...
describe('businessDay FEEL function', () => {
  test('moves Friday to Monday when the base date is an ISO string', () => {
    const engine = FeelExpressionEngine.create([ createBusinessDayFunction() ]);
  ...
  test('honours holiday and make-up-workday overrides', () => {
    ...
    const engine = FeelExpressionEngine.create([ createBusinessDayFunction(calendar) ]);

    expect(engine.evaluate('businessDay(date("2026-08-14"), 1)').value.toISODate())
      .toBe('2026-08-15');
    expect(engine.evaluate('businessDay(date("2026-08-15"), 1)').value.toISODate())
      .toBe('2026-08-18');
  });
